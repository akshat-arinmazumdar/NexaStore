  ...
  if (status === "loading") {
    return (
      <div className="min-h-screen bg-[#0F172A] flex flex-col items-center justify-center gap-6">
        ...
        <p className="text-slate-500 font-mono text-[10px] uppercase tracking-widest font-bold">Authenticating Admin Access...</p>
      </div>
    );
  }
  ...
  if ((session?.user as any)?.role !== "ADMIN") {
    return (
      <div className="min-h-screen bg-[#0F172A] flex flex-col items-center justify-center gap-6 text-center px-6">
        <div className="w-20 h-20 bg-red-500/10 rounded-full flex items-center justify-center text-red-500 border border-red-500/20 mb-4">
          <ShieldAlert className="w-10 h-10" />
        </div>
        <h2 className="text-3xl font-display font-bold text-white">Access Restricted</h2>
        <p className="text-slate-400 max-w-sm">This area is reserved for system administrators only. Redirecting you to safety...</p>
      </div>
    );
  }
